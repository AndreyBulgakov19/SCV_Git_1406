...
Для начала работы с удаленным репозиторием нужно его подключить к локальному. Сделать это можно двумя способами. 

Первый способ это инициализировать репозиторий с помощью `git clone`, в таком случае он будет уже подключен к удаленному, откуда производилось клонирование.

Второй способ это подключение удаленного репозитория командой `git remote add origin "ссылка на удаленный репозиторий"`.

С удаленного репозитория можно получать список веток и обновлять их состояние командой `git fetch`. Также можно переключаться на удаленные ветки командой `git checkout` и обновлять с них локальные командой `git pull`.

В случае желания выгрузить изменения на удаленный репозиторий, необходимо выполнить команду `git push`. Если же ветки в удаленном репозитории нет, то необходимо явно указать, что выгрузка будет происходить в новую ветку в удаленном репозитории. Для этого нужно использовать команду `git push -u origin "название ветки"`